 ...
 * @class TodoViews
 *
 * Representació visual del model.
 */
class TodoViews {
  ...
  public displayTodos(todos: { id: string; text: string; complete: boolean }[]): void {
  ...
  private _initLocalListeners(): void {
    this.todoList.addEventListener("input", event => {
      if (event.target && (event.target as HTMLElement).className === "editable") {
  ...
  public bindDeleteTodo(handler: (id: string) => void): void {
    this.todoList.addEventListener("click", event => {
      if (event.target && (event.target as HTMLElement).className === "delete") {
  ...
  public bindEditTodo(handler: (id: string, todoText: string) => void): void {
